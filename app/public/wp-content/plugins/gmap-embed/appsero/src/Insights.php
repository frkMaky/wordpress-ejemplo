...
class Insights
{
    ...
    protected function init_common()
    {
        ...
        add_action($this->client->slug . '_tracker_send_event_firsttime', [$this, 'send_tracking_data_firsttime']);
     ...
     * @return void
     */
    public function send_tracking_data_firsttime($override = false)
    {
        if (!$this->tracking_allowed() && !$override) {
            return;
        }

        // Request sent once opted-in(srm)
        $tracking_data = $this->get_tracking_data();
        $response = $this->client->send_request_firsttime($tracking_data, 'track');

        // Request sent once opted-in(appsero)
    ...
    /**
     * Schedule the event just after one minute  -one time
     *
     * @return void
     */
    private function schedule_event_firsttime()
    {
        $hook_name = wp_unslash($this->client->slug . '_tracker_send_event_firsttime');
        //will fire within one minute
        if (!wp_next_scheduled($hook_name)) {
            wp_schedule_single_event(time() + 10, $hook_name);
        }
    }
     ...
     * @return void
     */
    private function clear_schedule_event_firsttime()
    {
        wp_clear_scheduled_hook($this->client->slug . '_tracker_send_event_firsttime');
    }
    ...
    public function optin()
    {
        ...
        //Within one minute
        $this->clear_schedule_event_firsttime();
        $this->schedule_event_firsttime();
